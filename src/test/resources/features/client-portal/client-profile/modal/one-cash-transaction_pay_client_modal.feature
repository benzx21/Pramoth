...
Feature: Client portal
  ...
  Scenario Outline: Modal - Validate single cash PAY trough v1...v4 in Cash Transactions, Cash Positions and Ledger Postings screens via client profile
    ...
    And select on "Pending Cash" row 0
    ...
    And validate snapshot "c-cash_transaction_v1" on
      ...
      | <tradeSubType> | @date(T)     | [empty]           | @date(<valueDate>)     | <ccy>    | 0.00                | <baseCcy>          | <clientCode> | <clientMasterAccountName> | <subAccount> | 123456789                   | CPTYBIC1234      | CPTYCode          | NEW                | <direction> | <legalEntityCode>           | <amt>       | <amt*ratePref>                   | <nostroBank> | <nostroCode> | UNSETTLED         | CLIENT_UI | Test comment CT via csv Client portal |
    ...
    And validate snapshot "cash_transaction_v1" on
      | Booking Date | Contractual Value Date | Actual Value Date | Settlement Status | Transaction Status | Client code  | Master account            | Sub Account  | Nostro Code  | Nostro Bank  | Legal Entity Code (Custody) | Trade Sub-Type | Direction   | Cash Amount | Currency | Settled Cash Amount | Cash Amount (Preferred Currency) | Preferred Currency | FX Rate  | Counterparty Code | Counterparty BIC | Comment                               | Source    |
      | @date(T)     | @date(<valueDate>)     | [empty]           | UNSETTLED         | NEW                | <clientCode> | <clientMasterAccountName> | <subAccount> | <nostroCode> | <nostroBank> | <legalEntityCode>           | <tradeSubType> | <direction> | @abs(<amt>) | <ccy>    | 0.00                | @abs(<amt*ratePref>)             | <baseCcy>          | <fxRate> | CPTYCode          | CPTYBIC1234      | Test comment CT via csv Client portal | CLIENT_UI |
    ...
    And validate snapshot "cash_transaction_v1" against "cash_transaction_v2" on
      ...
      | ~   | @same        | @same                  | @date(<valueDate>) | SETTLED           | @same              | @same       | @same          | @same       | @same       | @same       | @same                       | @same          | @same     | @same       | @same    | @abs(<amt>)         | @same                            | @same              | @same   | @same             | @same            | @same   | @same  |
    ...
    And validate snapshot "c-cash_transaction_v1" against "c-cash_transaction_v2" on
      ...
      | ~   | @same          | @same        | @date(<valueDate>) | @same                  | @same    | @abs(<amt>)         | @same              | @same       | @same          | @same       | @same                       | @same            | @same             | @same              | @same     | @same                       | @same       | @same                            | @same       | @same       | SETTLED           | @same  | @same   |
    ...
    And validate snapshot "c-cash_position_v1" against "c-cash_position_v2" on
      ...
      | ~   | @same    | @same       | @same           | @same       | @same       | @same | <amt>           | 0.00            | 0.00           | @same                          |
      | ~   | @same    | @same       | @same           | @same       | @same       | @same | <amt*ratePref>  | 0.00            | 0.00           | @same                          |
    ...
    And validate snapshot "cash_transaction_v2" against "cash_transaction_v3" on
      | Ops | Booking Date | Contractual Value Date | Actual Value Date | Settlement Status | Transaction Status | Client code | Master account | Sub Account | Nostro Code | Nostro Bank | Legal Entity Code (Custody) | Trade Sub-Type | Direction | Cash Amount | Currency | Settled Cash Amount | Cash Amount (Preferred Currency) | Preferred Currency | FX Rate | Counterparty Code | Counterparty BIC | Comment | Source |
      | ~   | @same        | @same                  | [empty]           | UNSETTLED         | @same              | @same       | @same          | @same       | @same       | @same       | @same                       | @same          | @same     | @same       | @same    | 0.00                | @same                            | @same              | @same   | @same             | @same            | @same   | @same  |
    ...
    When filter on "Cash Transactions" by
      | Name        | Operator | Value        |
      | Sub Account | Equals   | <subAccount> |
    Then snapshot dataset "Cash Transactions" as "c-cash_transaction_v3"
    And validate snapshot "c-cash_transaction_v2" against "c-cash_transaction_v3" on
      | Ops | Trade Sub-Type | Booking Date | Actual Value Date | Contractual Value Date | Currency | Settled Cash Amount | Preferred Currency | Client code | Master account | Sub Account | Counterparty Account Number | Counterparty BIC | Counterparty Code | Transaction Status | Direction | Legal Entity Code (Custody) | Cash Amount | Cash Amount (Preferred Currency) | Nostro Bank | Nostro Code | Settlement Status | Source | Comment |
      | ~   | @same          | @same        | [empty]           | @same                  | @same    | 0.00                | @same              | @same       | @same          | @same       | @same                       | @same            | @same             | @same              | @same     | @same                       | @same       | @same                            | @same       | @same       | UNSETTLED         | @same  | @same   |
    ...
    And validate snapshot "c-cash_position_v2" against "c-cash_position_v3" on
      | Ops | COB Date | Client Code | Master Acc Name | Sub Account | Perspective | Ccy   | Settled Balance | Pending Credits     | Pending Debits      | Total Cash (Settled + Pending) |
      | ~   | @same    | @same       | @same           | @same       | @same       | @same | 0.00            | <cre_amt>           | <deb_amt>           | @same                          |
      | ~   | @same    | @same       | @same           | @same       | @same       | @same | 0.00            | <cre_amt*ratePrefT> | <deb_amt*ratePrefT> | @same                          |
    ...
    Then snapshot dataset "Cash Transactions" as "c-cash_transaction_v4"
    And validate snapshot "c-cash_transaction_v3" against "c-cash_transaction_v4" on
      | Ops | Trade Sub-Type | Booking Date | Actual Value Date | Contractual Value Date | Currency | Settled Cash Amount | Preferred Currency | Client code | Master account | Sub Account | Counterparty Account Number | Counterparty BIC | Counterparty Code | Transaction Status | Direction | Legal Entity Code (Custody) | Cash Amount | Cash Amount (Preferred Currency) | Nostro Bank | Nostro Code | Settlement Status | Source | Comment |
    ...
    And validate snapshot "cash_transaction_v3" against "cash_transaction_v4" on
      | Ops | Booking Date | Contractual Value Date | Actual Value Date | Settlement Status | Transaction Status | Client code | Master account | Sub Account | Nostro Code | Nostro Bank | Legal Entity Code (Custody) | Trade Sub-Type | Direction | Cash Amount | Currency | Settled Cash Amount | Cash Amount (Preferred Currency) | Preferred Currency | FX Rate | Counterparty Code | Counterparty BIC | Comment | Source |
      | ~   | @same        | @same                  | @same             | @same             | CANCELLED          | @same       | @same          | @same       | @same       | @same       | @same                       | @same          | @same     | @same       | @same    | @same               | @same                            | @same              | @same   | @same             | @same            | @same   | @same  |
    ...
    Examples:
      ...
      | c-env             | c-username        | c-password | c1-username        | c1-password | env              | username | password    | valueDate | tradeSubType | subAccount                      | baseCcy | direction | legalEntityCode | clientCode           | clientMasterAccountName   | nostroBank | nostroCode     | ccy | netConsideration | settDate | amt       | fxRate | amt*rate | amt*ratePref | deb_amt   | deb_amt*rateT | deb_amt*ratePrefT | cre_amt | cre_amt*rateT | cre_amt*ratePrefT | cre_cash_unSett_gl             | deb_cash_unSett_gl          | cre_cash_sett_gl | deb_cash_sett_gl               |
      | client-defaultEnv | JohnDoeCPCTClient | custx11*   | JohnDoeCPCTClient2 | custx11*    | admin-defaultEnv | JohnDoe  | Password11* | T         | Client Cash  | CP:Mod:SFL-GBP-BEL:PAY:CANCv4:T | GBP     | PAY       | SFL             | CP_CLIENT_CT_CC_Auto | CP_Auto_Testing_CT_Client | CITI       | CITINostroCode | GBP | 1000             | T        | -1,000.00 | 1.0000 | -820.00  | -1,000.00    | -1,000.00 | -820.00       | -1,000.00         | 0.00    | 0.00          | 0.00              | Free Cash 'In Transit' Account | Custody Client Cash Account | Nostro Account   | Free Cash 'In Transit' Account |
